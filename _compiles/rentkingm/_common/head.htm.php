<?php /* Template_ 2.2.8 2018/03/29 11:46:12 /home/rentking/dev/rentking/old/sites/tpls/rentkingm/_common/head.htm 000005262 */ ?>
<meta charset="UTF-8">
...
<link rel="stylesheet" href="/css/bootstrap.rentking.min.css?20171018025113<?php if(!$TPL_VAR["global"]["production"]){?>_<?php echo time()?><?php }?>">
<link rel="stylesheet" href="/css/bootstrap-datetimepicker.min.css">
<link rel="stylesheet" href="/css/rentking.m.min.css?20180329<?php if(!$TPL_VAR["global"]["production"]){?>_<?php echo time()?><?php }?>" >

<script type="text/javascript" src="/js/jquery-1.11.1.min.js"></script>
<script type="text/javascript" src="/js/moment-with-locales.min.js"></script>
<script type="text/javascript" src="/js/bootstrap.min.js"></script>
<script type="text/javascript" src="/js/bootstrap-datetimepicker.rentking.min.js?20171018025113<?php if(!$TPL_VAR["global"]["production"]){?>_<?php echo time()?><?php }?>"></script>
...
<script type="text/javascript" src="/js/rentking.m.min.js?20171127062239<?php if(!$TPL_VAR["global"]["production"]){?>_<?php echo time()?><?php }?>"></script>
...
<?
	if($TPL_VAR['global']['production']) {
		...
		include_once($_SERVER['DOCUMENT_ROOT']."/old/inc/plugins/acecounter_m.php");
...
<script type="text/javascript">
    ...
    CHPlugin.on('checkIn', function(err) {
        //console.log('channel.io checkin!');
    });

    CHPlugin.on('show', function(err) {
        ex2cts.push('track', 'button_onlineinquiry');
        console.log('button_onlineinquiry ex2cts.push');
    });

    $(document).ready(function(){
        $("li.call").on('click',function(e){
            ex2cts.push('track', 'button_call');
            console.log('button_call ex2cts.push');
        });
    });
</script>
<?
