	...
	<table class="listTableColor">
		...
		while($row = mysql_fetch_assoc($listr)) {?>
			<tr>
				...
				<td class="al" style="padding:0 4px;"><?=$row['msg']?></td>
			</tr>
			<?
			if($count == 0)
				$from = $row['idx'];
...
<script type="text/javascript">
	$(function() {
		...
		var getData = function() {
			$.getJSON('/monitoring/error.php?mode=ajax&from='+from, function(data) {
				if(data['count'] > 0) {
					...
					for(var i = 0; i < data['count']; i++) {
						html += '<tr  class="new">'
								...
								+ '<td class="al" style="padding:0 4px;">'+ data['data'][i]['msg'] + '</td>'
							+ '</tr>';
					}
